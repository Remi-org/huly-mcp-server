...
export interface MeasureContext<Q = any> {
    ...
    metrics?: Metrics;
    with: <T>(name: string, params: ParamsType, op: (ctx: MeasureContext<Q>) => T | Promise<T>, fullParams?: FullParamsType | (() => FullParamsType)) => Promise<T>;
    withSync: <T>(name: string, params: ParamsType, op: (ctx: MeasureContext<Q>) => T, fullParams?: FullParamsType | (() => FullParamsType)) => T;
    withLog: <T>(name: string, params: ParamsType, op: (ctx: MeasureContext<Q>) => T | Promise<T>, fullParams?: FullParamsType) => Promise<T>;
